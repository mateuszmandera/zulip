...
def process_emojis(
    record: dict[str, Any], import_dir: str, default_user_profile_id: int | None
) -> None:
    if not record["s3_path"].endswith(".original"):
        return
    ...
    assert record["s3_path"].endswith(".original")
...
def import_uploads(
    ...
    if processing_emojis and "author" in records[0]:
