...
def delete_expired_user_messages(realm: Realm, usermsg_ids: List[int]) -> None:
    ...
    removing_user_messages = UserMessage.objects.filter(
        ...
        message__pub_date__lt=(timezone_now() - timedelta(days=realm.message_retention_days)).isoformat()
    )
...
def move_expired_to_archive() -> Dict[str, Dict[str, List[int]]]:
    archived_data_info = {}  # type: Dict[str, Dict[str, List[int]]]
    for realm in Realm.objects.filter(message_retention_days__isnull=False).order_by("id"):
        ...
        archived_data_info[realm.string_id] = {
            'message_ids': msg_ids,
...
def clean_expired(archived_data_info: Dict[str, Dict[str, List[int]]]) -> None:
    for realm in Realm.objects.filter(message_retention_days__isnull=False).order_by("id"):
        delete_expired_user_messages(realm, archived_data_info[realm.string_id]['usermessage_ids'])
        delete_expired_messages(realm, archived_data_info[realm.string_id]['message_ids'])
