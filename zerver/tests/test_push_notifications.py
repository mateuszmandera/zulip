...
class PushBouncerNotificationTest(BouncerTestCase):
    ...
    def test_push_bouncer_api(self) -> None:
        ...
        with mock.patch(
            ...
            tokens = list(
                RemotePushDeviceToken.objects.filter(user_uuid=user.uuid(), server=server)
            )
            self.assert_length(tokens, 2)
    ...
    def test_end_to_end(self) -> None:
        ...
        with mock.patch(
            ...
            self.assertEqual(
                ...
                [
                    "INFO:zilencer.views:"
                    f"Sending mobile push notifications for remote user 6cde5f7a-1f7e-4978-9716-49f69ebfc9fe:uuid:{self.user_profile.uuid()}: "
            ...
            for _, _, token in apns_devices:
                self.assertIn(
                    ...
                    f"APNs: Success sending for user uuid:{self.user_profile.uuid()} to device {token}",
                    pn_logger.output,
                )
