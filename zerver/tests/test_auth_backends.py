...
class SAMLAuthBackendTest(SocialAuthBase):
    ...
    def test_saml_sp_initiated_logout_success(self) -> None:
        ...
        self.assertEqual(result.status_code, 302)
        self.assertIn(settings.SOCIAL_AUTH_SAML_ENABLED_IDPS["test_idp"]["slo_url"], result.url)
        ...
        query_dict = urllib.parse.parse_qs(urllib.parse.urlparse(result.url).query)
        ...
        self.assertEqual(result.status_code, 302)
        self.assertEqual(result.url, "/accounts/login/")
        self.client_get(result.url)
