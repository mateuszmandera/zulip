...
def fill_new_columns(apps: StateApps, schema_editor: DatabaseSchemaEditor) -> None:
    ...
    user_profile_id_list = [
        presence.user_profile_id for presence in latest_active_presence_per_user + latest_idle_presence_per_user
    ]
    user_id_to_realm_id = {user['id']: user['realm_id']
                           for user in UserProfile.objects.filter(id__in=user_profile_id_list)
                           .values('id', 'realm_id')}
    ...
    UserPresenceNew.objects.bulk_create(
        [UserPresenceNew(user_profile_id=user_id, realm_id=user_id_to_realm_id[user_id],
