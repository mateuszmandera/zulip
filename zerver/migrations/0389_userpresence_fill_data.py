...
def fill_new_columns(apps: StateApps, schema_editor: DatabaseSchemaEditor) -> None:
    UserPresenceOld = apps.get_model("zerver", "UserPresenceOld")
    ...
    latest_presence_per_user = (
        UserPresenceOld.objects.filter(
            status__in=[
...
class Migration(migrations.Migration):
    """
    Ports data from the old UserPresence model into the new one.
    """

    atomic = False

    dependencies = [
        ("zerver", "0388_userpresence_new_table_schema"),
    ]
